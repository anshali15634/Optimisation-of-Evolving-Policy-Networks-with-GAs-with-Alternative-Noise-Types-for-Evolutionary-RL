...
"""
    The GAMSM_P class implements the original framework for GA-MSM by Faycal et al. with a few novel modifications.
    Lines 108-114 onwards implement the novel modification as follows:
    - Only top 10% of the individuals different from the mutated population are preserved in the new population
    - Mutated population's elites are used to form the rest of the population.
    ...
    def run(self):
        ...
        while self.generation_number < self.number_of_generations:
            ...
            if self.fitness_calculated == True:
                population_fitness = mutated_population_fitness # reduces extra computation
            else:
            ...
            # crossover to create remaining population (not elite)
            ...
            # mutate that section of the population repeatedly number_of_additional_mutation number of times - excluding elites (need to be preserved)
            ...
            if sum(mutated_population_fitness) >= sum(population_fitness):
                ...
                new_elite_performers = labelled_new_indv_descending_fitness[:int(self.population_number * self.elite_percentage)]
                new_elite_individuals = [individual for individual, _ in new_elite_performers]

                population = [[] for _ in range(self.population_number)]
                
                # preserve new elites (most diverse individuals)
                population[:int(self.population_number * self.elite_percentage)]=copy.deepcopy(new_elite_individuals[:int(self.population_number*self.elite_percentage)])
                
                # use elites from mutated population to create rest of population
